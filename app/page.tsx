  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50">
      ...
      <section className="container mx-auto px-4 py-16 bg-white/40 backdrop-blur-sm">
        <div className="text-center mb-12">
          <h3 className="text-3xl font-bold text-emerald-900 mb-4">Our Past Work & Stories of Impact</h3>
        ...
        <div className="relative max-w-4xl mx-auto">
              ...
              {images.map((src, index) => (
                <div key={index} className="w-full flex-shrink-0 relative">
                  <img
                    ...
                    className="w-full h-150 object-cover bg-emerald-50"
                  />
                </div>
              ))}
            </div>
          ...
          <div className="flex justify-center mt-6 gap-2 flex-wrap">
            {images.map((_, index) => (
              <button
                key={index}
                className={`w-3 h-3 rounded-full transition-all duration-300 ${index === 0 ? 'bg-emerald-600' : 'bg-emerald-200 hover:bg-emerald-400'}`}
                id={`dot-${index}`}
              ></button>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div className="bg-white/80 rounded-2xl p-8 shadow-sm border border-emerald-50">
              ...
              <ul className="space-y-3 text-emerald-800">
                <li className="flex items-start gap-2"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500 mt-2 shrink-0"></div>Mantri Espana, Bellandur</li>
